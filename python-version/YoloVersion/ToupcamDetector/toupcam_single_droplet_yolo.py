...
# Add SingleDroplet directory to path to import exposure_control_panel
single_droplet_dir = os.path.join(python_version_dir, 'SingleDroplet')
sys.path.insert(0, single_droplet_dir)
...
# YOLO Model Path - using trained cell detection model
script_dir = os.path.dirname(os.path.abspath(__file__))
python_version_dir = os.path.dirname(os.path.dirname(script_dir))
...
def load_calibration_data():
    ...
    try:
        # Get path to calibration directory in python-version
        script_dir = os.path.dirname(os.path.abspath(__file__))
        python_version_dir = os.path.dirname(os.path.dirname(script_dir))
...
def refine_cell_boundary_with_contour(cell_patch, bbox_in_roi):
    """
    Refine cell boundary using contour detection within YOLO bounding box.
    Finds the most circular contour to get precise cell size.
    
    Args:
        cell_patch: Image patch of the cell (from YOLO bounding box)
        bbox_in_roi: Bounding box coordinates in ROI (x1, y1, x2, y2)
        
    Returns:
        Tuple of (refined_center, refined_radius, best_contour) or None if no good contour found
    """
    try:
        ...
        for contour in contours:
            ...
            # Only consider reasonably circular contours (> 0.5)
            if circularity > 0.5 and circularity > best_circularity:
                # Calculate center and radius from contour
        ...
        if best_contour is not None:
            # Convert center from patch coordinates to ROI coordinates
            x1, y1, x2, y2 = bbox_in_roi
            refined_cx = best_center[0] + x1
            refined_cy = best_center[1] + y1
            
            return ((refined_cx, refined_cy), best_radius, best_contour)
...
def detect_cells_in_roi(frame, roi_rect, params=None):
    """
    Detect cells within the specified region of interest using YOLOv11
    with contour-based refinement for precise size measurement.
    
    Args:
        frame: The input frame (BGR format)
        roi_rect: Rectangle defining the ROI (x, y, width, height)
        params: Optional parameters for cell detection (not used with YOLO, kept for compatibility)
        ...
        if results and len(results) > 0:
            ...
            if result.boxes is not None and len(result.boxes) > 0:
                ...
                for i, (box, conf) in enumerate(zip(boxes, confidences)):
                    ...
                    # Extract cell patch from ROI for contour refinement
                    patch_x1 = max(0, int(x1))
                    patch_y1 = max(0, int(y1))
                    patch_x2 = min(roi.shape[1], int(x2))
                    patch_y2 = min(roi.shape[0], int(y2))
                    
                    cell_patch = roi[patch_y1:patch_y2, patch_x1:patch_x2]
                    
                    # Try to refine with contour detection
                    refined = None
                    if cell_patch.size > 0:
                        refined = refine_cell_boundary_with_contour(
                            cell_patch, 
                            (patch_x1, patch_y1, patch_x2, patch_y2)
                        )
                    
                    if refined is not None:
                        # Use refined center and radius from contour
                        (cx_roi, cy_roi), radius, contour = refined
                        ...
                        # Create cell object with refined measurements
                        cell = DetectedCell((cx, cy), radius, contour, (bx, by, bw, bh))
                        detected_cells.append(cell)
                        # print(f"Cell {i}: Refined with contour (radius: {radius:.1f}px, circularity: good)")
                    else:
                        # Fallback to YOLO bounding box approximation
                        cx_roi = (x1 + x2) / 2
                        cy_roi = (y1 + y2) / 2
                        width = x2 - x1
                        height = y2 - y1
                        ...
                        # Create cell object with YOLO approximation
                        cell = DetectedCell((cx, cy), radius, None, (bx, by, bw, bh))
                        detected_cells.append(cell)
                        # print(f"Cell {i}: Using YOLO approximation (radius: {radius:.1f}px)")
                    
                print(f"YOLO detected {len(detected_cells)} cells in ROI")
        
        return detected_cells
        
    except Exception as e:
        print(f"Error in YOLO cell detection: {str(e)}")
        import traceback
        traceback.print_exc()
        return []
...
class SingleDropletApp:
    ...
    def __init__(self, root=None):
        ...
        if root is None:
            self.root = tk.Tk()
            self.root.title("ToupCam Single Droplet Cell Detection (YOLOv11)")
            self.owns_root = True
        else:
            self.root = root
    ...
    def create_ui(self):
        ...
        ttk.Button(control_frame, text="Reconnect Camera", command=self.reconnect_camera).pack(fill=tk.X, pady=5)
    ...
    def reconnect_camera(self):
        """Reconnect to the camera (useful if camera was disconnected or had errors)"""
        try:
            # Stop the current camera if it exists
            if self.hcam:
                try:
                    print("Stopping existing camera connection...")
                    self.running = False
                    self.hcam.Stop()
                    self.hcam.Close()
                    self.hcam = None
                    print("Existing camera stopped and closed")
                except Exception as e:
                    print(f"Error stopping existing camera: {e}")
                    self.hcam = None
            ...
            # Update status
            self.status_var.set("Reconnecting to camera...")
            ...
            if self.hcam:
                messagebox.showinfo("Camera Reconnected", "Camera reconnected successfully!")
            else:
                messagebox.showwarning("Reconnection Failed", "Failed to reconnect to camera. Please check camera connection.")
                
        except Exception as e:
            error_msg = f"Error reconnecting camera: {str(e)}"
            print(error_msg)
            messagebox.showerror("Reconnection Error", error_msg)
            self.status_var.set("Camera reconnection failed")
