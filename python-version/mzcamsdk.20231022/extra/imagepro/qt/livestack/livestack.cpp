...
void MainWidget::onBtnOpen()
{
    ...
    {
		m_hcam = Mzcam_Open(arr[0].id);
		if (m_hcam)
		{
			Mzcam_get_Size(m_hcam, &m_imgWidth, &m_imgHeight);
			Mzcam_put_Option(m_hcam, MZCAM_OPTION_BYTEORDER, 0); //Qimage use RGB byte order
            Mzcam_put_AutoExpoEnable(m_hcam, m_cbox_auto->isChecked() ? 1 : 0);
            ...
            if (FAILED(Mzcam_StartPullModeWithCallback(m_hcam, CameraCallBack, this)))
			{
				closeCamera();
...
void MainWidget::handleImageEvent()
{
    ...
    if (SUCCEEDED(Mzcam_PullImageV3(m_hcam, m_pVideoData, 0, 24, 0, &info)))
    {
