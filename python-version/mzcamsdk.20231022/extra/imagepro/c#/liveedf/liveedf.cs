...
internal class Liveedf : IDisposable
{
    ...
    public void Pull(Mzcam.SafeCamHandle hMzcam, int bFeed, IntPtr pImageData, int bits, int rowPitch, out Mzcam.FrameInfoV2 pInfo)
    {
        imagepro_edf_pull(handle_, hMzcam, bFeed, pImageData, bits, rowPitch, out pInfo);
    }

    public void PullV3(Mzcam.SafeCamHandle hMzcam, int bFeed, IntPtr pImageData, int bits, int rowPitch, out Mzcam.FrameInfoV3 pInfo)
    {
        imagepro_edf_pullV3(handle_, hMzcam, bFeed, pImageData, bits, rowPitch, out pInfo);
    }
    ...
    */
    ...
    [DllImport("imagepro.dll", ExactSpelling = true, CallingConvention = CallingConvention.Cdecl)]
    private static extern void imagepro_edf_pull(SafeLiveedfHandle handle, Mzcam.SafeCamHandle hMzcam, int bFeed, IntPtr pImageData, int bits, int rowPitch, out Mzcam.FrameInfoV2 pInfo);
    [DllImport("imagepro.dll", ExactSpelling = true, CallingConvention = CallingConvention.Cdecl)]
    private static extern void imagepro_edf_pullV3(SafeLiveedfHandle handle, Mzcam.SafeCamHandle hMzcam, int bFeed, IntPtr pImageData, int bits, int rowPitch, out Mzcam.FrameInfoV3 pInfo);
}