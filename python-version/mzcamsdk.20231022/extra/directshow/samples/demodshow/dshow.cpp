...
std::vector<TDshowDevice> dscap_enum_device()
{
	...
	if (g_pDshowEnumCamera)
	{
		...
		{
			TCHAR str[256];
			_stprintf(str, _T("Mzcam #%u"), i); /* Mzcam #0, Mzcam #1, etc */
...
HRESULT TDshowContext::InitCapFilter()
{
	...
	{
		unsigned nIndex = 0;
		const wchar_t* p = _tcschr(m_devname.c_str(), '#'); // extract the index from the name, such as 'Mzcam #0', 'Mzcam #1'
		if (p)
			nIndex = _ttoi(p + 1);
...
HRESULT TDshowContext::set_roi(unsigned xOffset, unsigned yOffset, unsigned xWidth, unsigned yHeight)
{
	...
	IMzcam* pIMzcam = NULL;
	if (m_pSource && SUCCEEDED(m_pSource->QueryInterface(IID_IMzcam, (void**)&pIMzcam)) && pIMzcam)
	{
		StopPreview();
		if (m_fPreviewGraphBuilt)
			TearDownGraph();
		hr = pIMzcam->put_Roi(xOffset, yOffset, xWidth, yHeight);
		pIMzcam->Release();
...
HRESULT TDshowContext::get_roi(unsigned* pxOffset, unsigned* pyOffset, unsigned* pxWidth, unsigned* pyHeight)
{
	...
	if (m_pSource && SUCCEEDED(m_pSource->QueryInterface(IID_IMzcam, (void**)&pIMzcam)) && pIMzcam)
	{
		hr = pIMzcam->get_Roi(pxOffset, pyOffset, pxWidth, pyHeight);
		pIMzcam->Release();
	}
	return hr;
}