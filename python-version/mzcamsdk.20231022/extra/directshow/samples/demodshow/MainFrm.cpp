...
void CMainFrame::OnExampleFliphorizontal()
{
	if (m_pDshowContext)
	{
		CComPtr<IMzcam> spIcam;
		if (SUCCEEDED(m_pDshowContext->queryinterface(IID_IMzcam, (void**)&spIcam)))
		{
			BOOL bHFlip;
...
void CMainFrame::OnSn()
{
	if (m_pDshowContext)
	{
		CComPtr<IMzcamSerialNumber> spISN;
		m_pDshowContext->queryinterface(IID_IMzcamSerialNumber, (void**)&spISN);
		if (spISN)
		{
			char sn[32];
