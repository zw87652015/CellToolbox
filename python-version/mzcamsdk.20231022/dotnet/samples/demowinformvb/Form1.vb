...
Public Class Form1
    Private Delegate Sub DelegateEvent(evt As Mzcam.eEVENT())
    Private cam_ As Mzcam = Nothing
    ...
    Private Sub OnEventImage()
        If bmp_ IsNot Nothing Then
            Dim info As New Mzcam.FrameInfoV3
    ...
    Private Sub DelegateOnEvent(evt As Mzcam.eEVENT())
        If cam_ IsNot Nothing Then
            Select Case (evt(0))
                Case Mzcam.eEVENT.EVENT_ERROR
                    OnEventError()
                Case Mzcam.eEVENT.EVENT_DISCONNECTED
                    OnEventDisconnected()
                Case Mzcam.eEVENT.EVENT_EXPOSURE
                    OnEventExposure()
                Case Mzcam.eEVENT.EVENT_IMAGE
                    OnEventImage()
                Case Mzcam.eEVENT.EVENT_STILLIMAGE
                    OnEventStillImage()
                Case Mzcam.eEVENT.EVENT_TEMPTINT
                    OnEventTempTint()
            End Select
        End If
    End Sub

    Private Sub DelegateOnEventCallback(evt As Mzcam.eEVENT)
        ' this is called by internal thread of mzcam.dll which Is Not the same of UI thread.
        ' Why we use BeginInvoke, Please see:
        ' http//msdn.microsoft.com/en-us/magazine/cc300429.aspx
        ' http://msdn.microsoft.com/en-us/magazine/cc188732.aspx
        ' http://stackoverflow.com/questions/1364116/avoiding-the-woes-of-invoke-begininvoke-in-cross-thread-winform-event-handling
        BeginInvoke(ev_, New Mzcam.eEVENT() {evt})
    End Sub
    ...
    Private Sub Form1_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Me.trackBar2.SetRange(Mzcam.TEMP_MIN, Mzcam.TEMP_MAX)
        Me.trackBar3.SetRange(Mzcam.TINT_MIN, Mzcam.TINT_MAX)
    End Sub
    ...
    Private Sub OnStart(sender As Object, e As EventArgs) Handles button1.Click
        ...
        Dim arr As Mzcam.DeviceV2() = Mzcam.[EnumV2]()
        ...
        If cam_ IsNot Nothing Then
            ...
            If cam_.get_eSize(eSize) Then
                ...
                If cam_.get_Size(width, height) Then
                    ...
                    If Not cam_.StartPullModeWithCallback(New Mzcam.DelegateEventCallback(AddressOf Me.DelegateOnEventCallback)) Then
            ...
            If cam_.get_eSize(eSize) Then
                If eSize <> comboBox1.SelectedIndex Then
                    ...
                    If cam_.get_Size(width, height) Then
                        bmp_ = New Bitmap(width, height, PixelFormat.Format24bppRgb)
                        ev_ = New DelegateEvent(AddressOf Me.DelegateOnEvent)
                        cam_.StartPullModeWithCallback(New Mzcam.DelegateEventCallback(AddressOf Me.DelegateOnEventCallback))
                    End If
                End If
            End If
        End If
