...
class MainWidget(QWidget):
    ...
    def __init__(self):
        ...
        self.pData = None
        self.res = 0
        self.temp = mzcam.MZCAM_TEMP_DEF
        self.tint = mzcam.MZCAM_TINT_DEF
        self.count = 0
        ...
        self.lbl_temp = QLabel(str(mzcam.MZCAM_TEMP_DEF))
        self.lbl_tint = QLabel(str(mzcam.MZCAM_TINT_DEF))
        self.slider_temp = QSlider(Qt.Horizontal)
        self.slider_tint = QSlider(Qt.Horizontal)
        self.slider_temp.setRange(mzcam.MZCAM_TEMP_MIN, mzcam.MZCAM_TEMP_MAX)
        self.slider_temp.setValue(mzcam.MZCAM_TEMP_DEF)
        self.slider_tint.setRange(mzcam.MZCAM_TINT_MIN, mzcam.MZCAM_TINT_MAX)
        self.slider_tint.setValue(mzcam.MZCAM_TINT_DEF)
    ...
    def startCamera(self):
        self.pData = bytes(mzcam.TDIBWIDTHBYTES(self.imgWidth * 24) * self.imgHeight)
        ...
        if self.cur.model.flag & mzcam.MZCAM_FLAG_MONO == 0:
            self.handleTempTintEvent()
        try:
            self.hcam.StartPullModeWithCallback(self.eventCallBack, self)
        except mzcam.HRESULTException:
            self.closeCamera()
            QMessageBox.warning(self, "Warning", "Failed to start camera.")
        else:
            self.cmb_res.setEnabled(True)
            self.cbox_auto.setEnabled(True)
            self.btn_autoWB.setEnabled(self.cur.model.flag & mzcam.MZCAM_FLAG_MONO == 0)
            self.slider_temp.setEnabled(self.cur.model.flag & mzcam.MZCAM_FLAG_MONO == 0)
            self.slider_tint.setEnabled(self.cur.model.flag & mzcam.MZCAM_FLAG_MONO == 0)
    ...
    def openCamera(self):
        self.hcam = mzcam.Mzcam.Open(self.cur.id)
        if self.hcam:
            ...
            with QSignalBlocker(self.cmb_res):
                ...
                self.cmb_res.setEnabled(True)
            self.hcam.put_Option(mzcam.MZCAM_OPTION_BYTEORDER, 0) #Qimage use RGB byte order
    ...
    def eventCallBack(nEvent, self):
        '''callbacks come from mzcam.dll/so internal threads, so we use qt signal to post this event to the UI thread'''
        self.evtCallback.emit(nEvent)

    def onevtCallback(self, nEvent):
        '''this run in the UI thread'''
        if self.hcam:
            if mzcam.MZCAM_EVENT_IMAGE == nEvent:
                self.handleImageEvent()
            elif mzcam.MZCAM_EVENT_EXPOSURE == nEvent:
                self.handleExpoEvent()
            elif mzcam.MZCAM_EVENT_TEMPTINT == nEvent:
                self.handleTempTintEvent()
            elif mzcam.MZCAM_EVENT_STILLIMAGE == nEvent:
                self.handleStillImageEvent()
            elif mzcam.MZCAM_EVENT_ERROR == nEvent:
                self.closeCamera()
    ...
    def handleStillImageEvent(self):
        ...
        try:
            ...
            if info.width > 0 and info.height > 0:
                buf = bytes(mzcam.TDIBWIDTHBYTES(info.width * 24) * info.height)
                try:
...
if __name__ == '__main__':
    mzcam.Mzcam.GigeEnable(None, None)
