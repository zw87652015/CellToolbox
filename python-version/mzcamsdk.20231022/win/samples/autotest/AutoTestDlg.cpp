...
BOOL CAutoTestDlg::OnInitDialog()
{
	...
	Mzcam_GigeEnable(GigeHotPlug, GetSafeHwnd());
...
void CAutoTestDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	else if (2 == nIDEvent)
	{
		...
		if (g_hcam)
		{
			...
			if (SUCCEEDED(Mzcam_get_FrameRate(g_hcam, &nFrame, &nTime, &nTotalFrame)) && nTime)
...
LRESULT CAutoTestDlg::OnPreviewResChanged(WPARAM wp, LPARAM lp)
{
	...
	HRESULT hr = Mzcam_get_eSize(g_hcam, &nResolutionIndex);
	if (FAILED(hr))
		return -1;

	if (nResolutionIndex != nSel)
	{
		hr = Mzcam_Stop(g_hcam);
		if (FAILED(hr))
			return -1;
		Mzcam_put_eSize(g_hcam, nSel);
...
{
	g_cameraCnt = Mzcam_EnumV2(g_cam);
...
void CAutoTestDlg::OnBnClickedButtonStart()
{
	...
	if (0 == startBtnText.Compare(_T("Open")))
	{
		...
		g_hcam = Mzcam_Open(g_cur.id);
		if (nullptr == g_hcam)
			return;

		m_dwHeartbeat = 0;
		if (g_HeartbeatTimeout && (g_cur.model->flag & MZCAM_FLAG_EVENT_HARDWARE))
		{
			Mzcam_put_Option(g_hcam, MZCAM_OPTION_EVENT_HARDWARE, 1);
			Mzcam_put_Option(g_hcam, MZCAM_OPTION_EVENT_HARDWARE | MZCAM_EVENT_HEARTBEAT, 1);

			SetTimer(1, 1000, nullptr);
		}
		Mzcam_put_Option(g_hcam, MZCAM_OPTION_NOPACKET_TIMEOUT, g_NopacketTimeout);
		Mzcam_put_Option(g_hcam, MZCAM_OPTION_NOFRAME_TIMEOUT, g_NoframeTimeout);
		Mzcam_put_RealTime(g_hcam, g_bRealtime ? 1 : 0);
...
void CAutoTestDlg::StartCamera()
{
	...
	Mzcam_get_Size(g_hcam, &width, &height);
	...
	Mzcam_StartPullModeWithWndMsg(g_hcam, m_hWnd, MSG_CAMEVENT);
}
...
void CAutoTestDlg::OnEventImage()
{
	...
	HRESULT hr = Mzcam_PullImageV3(g_hcam, m_pImageData, 0, 24, 0, &info);
	if (SUCCEEDED(hr))
	{
		if (g_bROITest && g_bROITest_SnapStart)
		{
			unsigned offsetX = 0, offsetY = 0, width = 0, height = 0;
			Mzcam_get_Roi(g_hcam, &offsetX, &offsetY, &width, &height);
		...
		else if (g_bImageSnap)
		{
			...
			Mzcam_get_Size(g_hcam, &resWidth, &resHeight);
			CString str;
			SYSTEMTIME tm;
			GetLocalTime(&tm);
...
{
	MzcamFrameInfoV3 info = { 0 };
	HRESULT hr = Mzcam_PullImageV3(g_hcam, nullptr, 1, 24, 0, &info);
	if (SUCCEEDED(hr))
	{
		void* pData = malloc(TDIBWIDTHBYTES(info.width * 24) * info.height);
		hr = Mzcam_PullImageV3(g_hcam, pData, 1, 24, 0, nullptr);
		if (SUCCEEDED(hr))
		{
			BITMAPINFOHEADER header = { 0 };
			header.biSize = sizeof(header);
...
void CAutoTestDlg::OnBnClickedButton1()
{
	...
	Mzcam_Replug(g_cam[m_camList.GetCurSel()].id);
}
...
void COptionsDlg::OnOK()
{
	...
	if (g_NopacketTimeout && (g_NopacketTimeout < MZCAM_NOPACKET_TIMEOUT_MIN))
	{
	...
	if (g_NoframeTimeout && (g_NoframeTimeout < MZCAM_NOFRAME_TIMEOUT_MIN))
	{
