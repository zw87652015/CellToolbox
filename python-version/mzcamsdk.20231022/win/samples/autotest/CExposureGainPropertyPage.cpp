...
void CExposureGainPropertyPage::OnAutoExposure()
{
	if (GetSafeHwnd())
	{
		...
		{
			unsigned time = 0;
			Mzcam_get_ExpoTime(g_hcam, &time);
		...
		if (GetDlgItem(IDC_SLIDER_GAIN))
		{
			...
			Mzcam_get_ExpoAGain(g_hcam, &gain);
			SetGainValue(gain);
		}
	}
}
...
{
	int bAutoExp = 0;
	Mzcam_get_AutoExpoEnable(g_hcam, &bAutoExp);
	...
	((CSliderCtrl*)GetDlgItem(IDC_SLIDER_TARGET))->SetRange(MZCAM_AETARGET_MIN, MZCAM_AETARGET_MAX);
	unsigned short target = 0;
	Mzcam_get_AutoExpoTarget(g_hcam, &target);
	SetTargetValue(target);

	unsigned timeMin = 0, timeMax = 0, timeDef = 0, timeVal = 0;
	Mzcam_get_ExpTimeRange(g_hcam, &timeMin, &timeMax, &timeDef);
	((CSliderCtrl*)GetDlgItem(IDC_SLIDER_EXPOSURE))->SetRange(timeMin, timeMax);
	Mzcam_get_ExpoTime(g_hcam, &timeVal);
	SetExpoTimeValue(timeVal);

	unsigned short gainMin = 0, gainMax = 0, gainDef = 0, gainVal = 0;
	Mzcam_get_ExpoAGainRange(g_hcam, &gainMin, &gainMax, &gainDef);
	((CSliderCtrl*)GetDlgItem(IDC_SLIDER_GAIN))->SetRange(gainMin, gainMax);
	Mzcam_get_ExpoAGain(g_hcam, &gainVal);
...
{
	Mzcam_put_AutoExpoEnable(g_hcam, ((CButton*)GetDlgItem(IDC_CHECK_AUTO))->GetCheck() ? 1 : 0);
	UpdateSlidersEnable();
}
...
void CExposureGainPropertyPage::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	if (pScrollBar == GetDlgItem(IDC_SLIDER_TARGET))
	{
		...
		Mzcam_get_AutoExpoTarget(g_hcam, &curTarget);
		unsigned short target = ((CSliderCtrl*)GetDlgItem(IDC_SLIDER_TARGET))->GetPos();
		if (target != curTarget)
		{
			Mzcam_put_AutoExpoTarget(g_hcam, target);
	...
	{
		unsigned curTime = 0;
		Mzcam_get_ExpoTime(g_hcam, &curTime);
		unsigned time = ((CSliderCtrl*)GetDlgItem(IDC_SLIDER_EXPOSURE))->GetPos();
		if (time != curTime)
		{
			Mzcam_put_ExpoTime(g_hcam, time);
			CString str;
	...
	else if (pScrollBar == GetDlgItem(IDC_SLIDER_GAIN))
	{
		...
		Mzcam_get_ExpoAGain(g_hcam, &curGain);
		unsigned short gain = ((CSliderCtrl*)GetDlgItem(IDC_SLIDER_GAIN))->GetPos();
		if (gain != curGain)
		{
			Mzcam_put_ExpoAGain(g_hcam, gain);
