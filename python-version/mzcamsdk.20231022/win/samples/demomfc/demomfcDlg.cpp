...
BEGIN_MESSAGE_MAP(CdemomfcDlg, CDialog)
	...
	ON_WM_HSCROLL()
	ON_COMMAND_RANGE(IDM_SNAP_RESOLUTION, IDM_SNAP_RESOLUTION + MZCAM_MAX, &CdemomfcDlg::OnSnapResolution)
	ON_WM_LBUTTONDOWN()
	ON_WM_SETCURSOR()
END_MESSAGE_MAP()
...
void CdemomfcDlg::OnBnClickedButton1()
{
	...
	const int n = (int)Mzcam_get_ResolutionNumber(m_hcam);
	if (n > 0)
	{
		TCHAR txt[128];
		int nWidth, nHeight;
		for (int i = 0; i < n; ++i)
		{
			Mzcam_get_Resolution(m_hcam, i, &nWidth, &nHeight);
			_stprintf(txt, _T("%d * %d"), nWidth, nHeight);
			pCombox->AddString(txt);
		}

		unsigned nCur = 0;
		Mzcam_get_eSize(m_hcam, &nCur);
...
void CdemomfcDlg::StartDevice()
{
	...
	HRESULT hr = Mzcam_get_Size(m_hcam, &nWidth, &nHeight);
	if (FAILED(hr))
		return;
	...
	Mzcam_StartPullModeWithWndMsg(m_hcam, m_hWnd, MSG_CAMEVENT);

	BOOL bEnableAutoExpo = TRUE;
	Mzcam_get_AutoExpoEnable(m_hcam, &bEnableAutoExpo);
	...
	Mzcam_get_ExpTimeRange(m_hcam, &nMinExpoTime, &nMaxExpoTime, &nDefExpoTime);
	((CSliderCtrl*)GetDlgItem(IDC_SLIDER1))->SetRange(nMinExpoTime / 1000, nMaxExpoTime / 1000);
	((CSliderCtrl*)GetDlgItem(IDC_SLIDER2))->SetRange(MZCAM_TEMP_MIN, MZCAM_TEMP_MAX);
	((CSliderCtrl*)GetDlgItem(IDC_SLIDER3))->SetRange(MZCAM_TINT_MIN, MZCAM_TINT_MAX);
...
void CdemomfcDlg::OnCbnSelchangeCombo1()
{
	...
	HRESULT hr = Mzcam_get_eSize(m_hcam, &nResolutionIndex);
	if (FAILED(hr))
		return;

	if (nResolutionIndex != nSel)
	{
		hr = Mzcam_Stop(m_hcam);
		if (FAILED(hr))
			return;

		Mzcam_put_eSize(m_hcam, nSel);
...
{
	unsigned nTime = 0;
	Mzcam_get_ExpoTime(m_hcam, &nTime);
...
{
	HRESULT hr = Mzcam_PullImageV2(m_hcam, m_pImageData, 24, NULL);
	if (SUCCEEDED(hr))
	{
		const CRect rc = GetDrawRect();
		CClientDC dc(this);
...
void CdemomfcDlg::OnEventStillImage()
{
	...
	HRESULT hr = Mzcam_PullImageV3(m_hcam, NULL, 1, 24, 0, &info);
	if (SUCCEEDED(hr))
	{
		void* pData = malloc(TDIBWIDTHBYTES(info.width * 24) * info.height);
		hr = Mzcam_PullImageV3(m_hcam, pData, 1, 24, 0, NULL);
		if (SUCCEEDED(hr))
		{
...
void CdemomfcDlg::GetAEAuxRect()
{
	const CRect rc = GetDrawRect();
	RECT rect;
	Mzcam_get_AEAuxRect(m_hcam, &rect);
...
void CdemomfcDlg::SetAEAuxRect()
{
	...
	Mzcam_put_AEAuxRect(m_hcam, &rect);
}
...
{
	Mzcam_Snap(m_hcam, nID - IDM_SNAP_RESOLUTION);
}

void CdemomfcDlg::OnBnClickedButton2()
{
	const int n = Mzcam_get_StillResolutionNumber(m_hcam);
	if (n <= 0)
		Mzcam_Snap(m_hcam, 0xffffffff);
	else
	{
		CPoint pt;
		GetCursorPos(&pt);
		...
		for (int i = 0; i < n; ++i)
		{
			Mzcam_get_StillResolution(m_hcam, i, &w, &h);
...
{
	if (m_hcam)
		Mzcam_put_AutoExpoEnable(m_hcam, IsDlgButtonChecked(IDC_CHECK1) ? 1 : 0);
...
void CdemomfcDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	if (pScrollBar == GetDlgItem(IDC_SLIDER1))
	{
		...
		Mzcam_put_ExpoTime(m_hcam, nTime * 1000);
	}
	else
	{
		...
		SetDlgItemInt(IDC_STATIC2, nTemp, TRUE);
		SetDlgItemInt(IDC_STATIC3, nTint, TRUE);
		Mzcam_put_TempTint(m_hcam, nTemp, nTint);
	}
