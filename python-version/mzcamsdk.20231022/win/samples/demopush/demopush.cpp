...
public:
	...
	int OnCreate(LPCREATESTRUCT /*lpCreateStruct*/)
	{
		...
		unsigned cnt = Mzcam_EnumV2(m_dev);
		if (0 == cnt)
	...
	void OnDataCallbackV4(const void* pData, const MzcamFrameInfoV3* pInfo, int bSnap)
	{
		if (NULL == pData)
			PostMessage(MSG_CAMEVENT, MZCAM_EVENT_ERROR);
		else if (bSnap)
		{
	...
	static void __stdcall StaticOnDataCallbackV4(const void* pData, const MzcamFrameInfoV3* pInfo, int bSnap, void* pCallbackCtx)
	{
			...
			if (eSize != nID - ID_PREVIEW_RESOLUTION0)
			{
				if (SUCCEEDED(Mzcam_Stop(m_hcam)))
				{
					...
					if (SUCCEEDED(Mzcam_get_Size(m_hcam, (int*)&m_header.biWidth, (int*)&m_header.biHeight)))
					{
						...
						Mzcam_StartPushModeV4(m_hcam, StaticOnDataCallbackV4, this, StaticOnEventCallback, this);
					}
				}
			}
		}
	...
	void OnOpenDevice(UINT /*uNotifyCode*/, int nID, HWND /*wndCtl*/)
	{
		...
		m_hcam = Mzcam_Open(m_dev[m_nIndex].id);
		if (m_hcam)
		{
