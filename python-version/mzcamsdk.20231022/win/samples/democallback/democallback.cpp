...
public:
	...
	void OnDataEvent(bool bSnap)
	{
		if (bSnap)
		{
			...
			Mzcam_PullImageV3(m_hcam, NULL, 1, 24, 0, &info);
			...
			if (pSnapData)
			{
				Mzcam_PullImageV3(m_hcam, pSnapData, 1, 24, 0, NULL);
		...
		{
			{
				...
				Mzcam_PullImageV3(m_hcam, m_pData, 0, 24, 0, NULL);
			}
			...
			if (eSize != nID - ID_PREVIEW_RESOLUTION0)
			{
				if (SUCCEEDED(Mzcam_Stop(m_hcam)))
				{
					...
					if (SUCCEEDED(Mzcam_get_Size(m_hcam, (int*)&m_header.biWidth, (int*)&m_header.biHeight)))
					{
						...
						Mzcam_StartPullModeWithCallback(m_hcam, StaticOnEventCallback, this);
					}
				}
			}
		}
