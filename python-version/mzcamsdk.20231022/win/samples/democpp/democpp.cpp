...
private:
	LRESULT OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		if (SUCCEEDED(Mzcam_get_ExpTimeRange(m_hcam, &nMin, &nMax, &nDef)))
		{
			CTrackBarCtrl ctrl(GetDlgItem(IDC_SLIDER1));
			ctrl.SetRangeMin(nMin);
			ctrl.SetRangeMax(nMax);

			if (SUCCEEDED(Mzcam_get_ExpoTime(m_hcam, &nTime)))
				ctrl.SetPos(nTime);
		}
	...
	{
		CTrackBarCtrl ctrl(GetDlgItem(IDC_SLIDER1));
		Mzcam_put_ExpoTime(m_hcam, ctrl.GetPos());
...
private:
	LRESULT OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		CTrackBarCtrl ctrl(GetDlgItem(IDC_SLIDER1));
		ctrl.SetRangeMin(0);
		ctrl.SetRangeMax(Mzcam_get_MaxSpeed(m_hcam));

		unsigned short nSpeed = 0;
		if (SUCCEEDED(Mzcam_get_Speed(m_hcam, &nSpeed)))
			ctrl.SetPos(nSpeed);
	...
	{
		CTrackBarCtrl ctrl(GetDlgItem(IDC_SLIDER1));
		Mzcam_put_Speed(m_hcam, ctrl.GetPos());
...
private:
	...
	LRESULT OnOK(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (bTran1 && bTran2)
			Mzcam_put_MaxAutoExpoTimeAGain(m_hcam, nTime, nGain);
		EndDialog(wID);
		return 0;
	}
...
private:
	...
	LRESULT OnButton1(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		Mzcam_put_LEDState(m_hcam, nIndex, 1, 0);
		return 0;
	}
	...
	LRESULT OnButton2(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		Mzcam_put_LEDState(m_hcam, nIndex, 2, nPeriod);
		return 0;
	}
	...
	{
		const UINT nIndex = GetDlgItemInt(IDC_EDIT1);
		Mzcam_put_LEDState(m_hcam, nIndex, 0, 0);
		return 0;
	}
...
public:
	...
	CPixelFormatDlg(const MzcamDeviceV2& tdev, HMzcam hcam)
...
private:
	LRESULT OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		if (0 == (m_inst.model->flag & (MZCAM_FLAG_RAW8 | MZCAM_FLAG_GMCY8)))
			GetDlgItem(IDC_RADIO1).EnableWindow(FALSE);
		
		if (m_inst.model->flag & (MZCAM_FLAG_RAW10 | MZCAM_FLAG_RAW12 | MZCAM_FLAG_RAW14 | MZCAM_FLAG_RAW16 | MZCAM_FLAG_GMCY12))
		{
			wchar_t str[16];
			unsigned bits = 16;
			if (m_inst.model->flag & MZCAM_FLAG_RAW10)
				bits = 10;
			else if (m_inst.model->flag & (MZCAM_FLAG_RAW12 | MZCAM_FLAG_GMCY12))
				bits = 12;
			else if (m_inst.model->flag & MZCAM_FLAG_RAW14)
				bits = 14;
		...
		if (0 == (m_inst.model->flag & (MZCAM_FLAG_VUYY | MZCAM_FLAG_UYVY)))
			GetDlgItem(IDC_RADIO3).EnableWindow(FALSE);
		if (0 == (m_inst.model->flag & MZCAM_FLAG_RAW12PACK))
			GetDlgItem(IDC_RADIO4).EnableWindow(FALSE);

		int val = 0;
		Mzcam_get_Option(m_hcam, MZCAM_OPTION_PIXEL_FORMAT, &val);
		if ((MZCAM_PIXELFORMAT_RAW8 == val) || (MZCAM_PIXELFORMAT_GMCY8 == val))
			CheckDlgButton(IDC_RADIO1, 1);
		else if ((MZCAM_PIXELFORMAT_RAW10 == val) || (MZCAM_PIXELFORMAT_RAW12 == val) || (MZCAM_PIXELFORMAT_RAW14 == val) || (MZCAM_PIXELFORMAT_RAW16 == val) || (MZCAM_PIXELFORMAT_GMCY12 == val))
			CheckDlgButton(IDC_RADIO2, 1);
		else if ((MZCAM_PIXELFORMAT_VUYY == val) || (MZCAM_PIXELFORMAT_UYVY == val))
			CheckDlgButton(IDC_RADIO3, 1);
		else if (MZCAM_PIXELFORMAT_RAW12PACK == val)
	...
	{
		if (m_inst.model->flag & MZCAM_FLAG_RAW8)
			Mzcam_put_Option(m_hcam, MZCAM_OPTION_PIXEL_FORMAT, MZCAM_PIXELFORMAT_RAW8);
		else if (m_inst.model->flag & MZCAM_FLAG_GMCY8)
			Mzcam_put_Option(m_hcam, MZCAM_OPTION_PIXEL_FORMAT, MZCAM_PIXELFORMAT_GMCY8);
		return 0;
	}
	...
	{
		if (m_inst.model->flag & MZCAM_FLAG_RAW10)
			Mzcam_put_Option(m_hcam, MZCAM_OPTION_PIXEL_FORMAT, MZCAM_PIXELFORMAT_RAW10);
		else if (m_inst.model->flag & MZCAM_FLAG_RAW12)
			Mzcam_put_Option(m_hcam, MZCAM_OPTION_PIXEL_FORMAT, MZCAM_PIXELFORMAT_RAW12);
		else if (m_inst.model->flag & MZCAM_FLAG_RAW14)
			Mzcam_put_Option(m_hcam, MZCAM_OPTION_PIXEL_FORMAT, MZCAM_PIXELFORMAT_RAW14);
		else if (m_inst.model->flag & MZCAM_FLAG_RAW16)
			Mzcam_put_Option(m_hcam, MZCAM_OPTION_PIXEL_FORMAT, MZCAM_PIXELFORMAT_RAW16);
		else if (m_inst.model->flag & MZCAM_FLAG_GMCY12)
			Mzcam_put_Option(m_hcam, MZCAM_OPTION_PIXEL_FORMAT, MZCAM_PIXELFORMAT_GMCY12);
		return 0;
	}

	LRESULT OnRadio3(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		if (m_inst.model->flag & MZCAM_FLAG_VUYY)
			Mzcam_put_Option(m_hcam, MZCAM_OPTION_PIXEL_FORMAT, MZCAM_PIXELFORMAT_VUYY);
		else if (m_inst.model->flag & MZCAM_FLAG_UYVY)
			Mzcam_put_Option(m_hcam, MZCAM_OPTION_PIXEL_FORMAT, MZCAM_PIXELFORMAT_UYVY);
		return 0;
	}

	LRESULT OnRadio4(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		if (m_inst.model->flag & MZCAM_FLAG_RAW12PACK)
			Mzcam_put_Option(m_hcam, MZCAM_OPTION_PIXEL_FORMAT, MZCAM_PIXELFORMAT_RAW12PACK);
		return 0;
	}
...
private:
	LRESULT OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		int val = 0;
		Mzcam_get_Option(m_hcam, MZCAM_OPTION_TECTARGET, &val);
	...
	LRESULT OnOK(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		Mzcam_put_Option(m_hcam, MZCAM_OPTION_TECTARGET, (int)(d * 10));
	...
	LRESULT OnButton1(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (GetDlgItemText(IDC_EDIT1, strAddr, _countof(strAddr)) && GetDlgItemText(IDC_EDIT2, strLength, _countof(strLength)))
		{
			...
			if (uLength)
			{
				...
				HRESULT hr = Mzcam_read_EEPROM(m_hcam, uAddr, tmpBuffer, uLength);
				if (FAILED(hr))
	...
	LRESULT OnButton2(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (GetDlgItemText(IDC_EDIT1, strAddr, _countof(strAddr)) && GetDlgItemText(IDC_EDIT2, strLength, _countof(strLength)) && GetDlgItemText(IDC_EDIT3, strData, _countof(strData)))
		{
			...
			if (uLength)
			{
				...
				const HRESULT hr = Mzcam_write_EEPROM(m_hcam, uAddr, tmpBuffer, uLength);
	...
	LRESULT OnButton3(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (GetDlgItemText(IDC_EDIT1, strAddr, _countof(strAddr)) && GetDlgItemText(IDC_EDIT2, strLength, _countof(strLength)))
		{
			...
			if (uLength)
			{
				...
				const HRESULT hrWrite = Mzcam_write_EEPROM(m_hcam, uAddr, tmpWriteBuffer, uLength);
				const HRESULT hrRead = Mzcam_read_EEPROM(m_hcam, uAddr, tmpReadBuffer, uLength);
	...
	LRESULT OnButton4(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (GetDlgItemText(IDC_EDIT1, strAddr, _countof(strAddr)) && GetDlgItemText(IDC_EDIT2, strLength, _countof(strLength)))
		{
			...
			if (uLength)
			{
				...
				const HRESULT hr = Mzcam_write_EEPROM(m_hcam, uAddr, tmpWriteBuffer, uLength);
				if (hr == uLength)
				{
...
private:
	...
	LRESULT OnWmTimer(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		if (1 == wParam)
		{
			if (S_OK == Mzcam_rwc_Flash(m_hcam, MZCAM_FLASH_STATUS, 0, 0, NULL))
				EndDialog(IDOK);
			else
...
public:
	enum { IDD = IDD_FLASH };
	CFlashDlg(HMzcam hcam)
	: m_hcam(hcam)
	, m_totalSize(Mzcam_rwc_Flash(hcam, MZCAM_FLASH_SIZE, 0, 0, NULL))
	, m_eBlock(Mzcam_rwc_Flash(hcam, MZCAM_FLASH_EBLOCK, 0, 0, NULL))
	, m_rwBlock(Mzcam_rwc_Flash(hcam, MZCAM_FLASH_RWBLOCK, 0, 0, NULL))
	{
	}
private:
	LRESULT OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	...
	LRESULT OnButton1(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (GetDlgItemText(IDC_EDIT1, strAddr, _countof(strAddr)) && GetDlgItemText(IDC_EDIT2, strLength, _countof(strLength)))
		{
			...
			if (uLength)
			{
				...
				{
					unsigned char* tmpBuffer = (unsigned char*)alloca(uLength);
					HRESULT hr = Mzcam_rwc_Flash(m_hcam, MZCAM_FLASH_READ, uAddr, uLength, tmpBuffer);
					if (FAILED(hr))
					{
						wchar_t strText[256];
		...
		if (GetDlgItemText(IDC_EDIT1, strAddr, _countof(strAddr)) && GetDlgItemText(IDC_EDIT2, strLength, _countof(strLength)) && GetDlgItemText(IDC_EDIT3, strData, _countof(strData)))
		{
			...
			if (uLength)
			{
				...
				else
				{
					...
					const HRESULT hr = Mzcam_rwc_Flash(m_hcam, MZCAM_FLASH_WRITE, uAddr, uLength, tmpBuffer);
					if (FAILED(hr))
					{
						wchar_t strText[256];
		...
		if (GetDlgItemText(IDC_EDIT1, strAddr, _countof(strAddr)) && GetDlgItemText(IDC_EDIT2, strLength, _countof(strLength)))
		{
			...
			if (uLength)
			{
				...
				else
				{
					...
					for (unsigned i = 0; i < uLength; ++i)
						tmpWriteBuffer[i] = (unsigned char)rand();
					HRESULT hr = Mzcam_rwc_Flash(m_hcam, MZCAM_FLASH_WRITE, uAddr, uLength, tmpWriteBuffer);
					if (FAILED(hr))
					{
						swprintf(strText, L"Write failed, hr = 0x%08x", hr);
						AtlMessageBox(m_hWnd, strText, MB_OK | MB_ICONERROR);
					...
					else
					{
						...
						if (IDOK == dlg.DoModal())
						{
							hr = Mzcam_rwc_Flash(m_hcam, MZCAM_FLASH_READ, uAddr, uLength, tmpReadBuffer);
							if (FAILED(hr))
							{
								swprintf(strText, L"Read failed, hr = 0x%08x", hr);
								AtlMessageBox(m_hWnd, strText, MB_OK | MB_ICONERROR);
	...
	LRESULT OnButton4(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (GetDlgItemText(IDC_EDIT1, strAddr, _countof(strAddr)) && GetDlgItemText(IDC_EDIT2, strLength, _countof(strLength)))
		{
			...
			if (uLength)
			{
				...
				{
					const HRESULT hr = Mzcam_rwc_Flash(m_hcam, MZCAM_FLASH_ERASE, uAddr, uLength, NULL);
					if (FAILED(hr))
					{
						wchar_t strText[256];
...
private:
	...
	LRESULT OnButton1(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (GetDlgItemText(IDC_EDIT2, strLength, _countof(strLength)))
		{
			...
			{
				unsigned char* tmpBuffer = (unsigned char*)alloca(uLength);
				HRESULT hr = Mzcam_read_UART(m_hcam, tmpBuffer, uLength);
				if (FAILED(hr))
	...
	LRESULT OnButton2(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (GetDlgItemText(IDC_EDIT2, strLength, _countof(strLength)) && GetDlgItemText(IDC_EDIT3, strData, _countof(strData)))
		{
			...
			if (uLength)
			{
				...
				HRESULT hr = Mzcam_write_UART(m_hcam, tmpBuffer, uLength);
				wchar_t strMessage[256];
...
public:
	...
	CIocontrolDlg(HMzcam hcam, const MzcamDeviceV2& tdev)
...
private:
	...
	{
		Mzcam_IoControl(m_hcam, 0, MZCAM_IOCONTROLTYPE_SET_RESETCOUNTER, 0, NULL);
		return 0;
	}
	...
	LRESULT OnOK(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		}
		{
			CComboBox box(GetDlgItem(IDC_GPIODIR));
			if (0 == box.GetCurSel())
				Mzcam_IoControl(m_hcam, index, MZCAM_IOCONTROLTYPE_SET_GPIODIR, 0x00, NULL);
			else
				Mzcam_IoControl(m_hcam, index, MZCAM_IOCONTROLTYPE_SET_GPIODIR, 0x01, NULL);
		}
		{
			CComboBox box(GetDlgItem(IDC_OUTPUTINVERTER));
			Mzcam_IoControl(m_hcam, index, MZCAM_IOCONTROLTYPE_SET_OUTPUTINVERTER, box.GetCurSel(), NULL);
		}
		{
			CComboBox box(GetDlgItem(IDC_INPUTACTIVATION));
			Mzcam_IoControl(m_hcam, index, MZCAM_IOCONTROLTYPE_SET_INPUTACTIVATION, box.GetCurSel(), NULL);
		}
		{
			CString str;
			...
			if (!str.IsEmpty())
			{
				...
				Mzcam_IoControl(m_hcam, 0, MZCAM_IOCONTROLTYPE_SET_STROBEDURATION, val, NULL);
			}
		}
		{
			CString str;
			...
			{
				const int val = _ttoi((LPCTSTR)str);
				Mzcam_IoControl(m_hcam, index, MZCAM_IOCONTROLTYPE_SET_DEBOUNCERTIME, val, NULL);
			}
		}
		{
			CString str;
			...
			{
				const int val = _ttoi((LPCTSTR)str);
				Mzcam_IoControl(m_hcam, 0, MZCAM_IOCONTROLTYPE_SET_COUNTERVALUE, val, NULL);
			}
		}
		{
			CComboBox box(GetDlgItem(IDC_OUTPUTMODE));
			Mzcam_IoControl(m_hcam, index, MZCAM_IOCONTROLTYPE_SET_OUTPUTMODE, box.GetCurSel(), NULL);
		}
		{
			CString str;
			...
			{
				const int val = _ttoi((LPCTSTR)str);
				Mzcam_IoControl(m_hcam, 0, MZCAM_IOCONTROLTYPE_SET_USERVALUE, val, NULL);
			}
		}
		{
			CComboBox box(GetDlgItem(IDC_STROBEDELAYMODE));
			Mzcam_IoControl(m_hcam, 0, MZCAM_IOCONTROLTYPE_SET_STROBEDELAYMODE, box.GetCurSel(), NULL);
		}
		{
			CString str;
			...
			{
				const int val = _ttoi((LPCTSTR)str);
				Mzcam_IoControl(m_hcam, 0, MZCAM_IOCONTROLTYPE_SET_STROBEDELAYTIME, val, NULL);
			}
		}
		{
			CString str;
			...
			{
				const int val = _ttoi((LPCTSTR)str);
				Mzcam_IoControl(m_hcam, 0, MZCAM_IOCONTROLTYPE_SET_TRIGGERDELAY, val, NULL);
			}
		}
		{
			CComboBox box(GetDlgItem(IDC_PWMSOURCE));
			Mzcam_IoControl(m_hcam, index, MZCAM_IOCONTROLTYPE_SET_PWMSOURCE, box.GetCurSel(), NULL);
		}
		return 0;
	}
};
...
class CMainFrame : public CFrameWindowImpl<CMainFrame>, public CUpdateUI<CMainFrame>
{
	HMzcam		m_hcam;
	CMainView		m_view;
	MzcamDeviceV2	m_arrDev[MZCAM_MAX], m_dev;
	BOOL			m_bPaused;
...
private:
	LRESULT OnMsgCamEvent(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		switch (wParam)
		{
		case MZCAM_EVENT_ERROR:
		case MZCAM_EVENT_NOFRAMETIMEOUT:
		case MZCAM_EVENT_NOPACKETTIMEOUT:
			OnEventError();
			break;
		case MZCAM_EVENT_DISCONNECTED:
			OnEventDisconnected();
			break;
		case MZCAM_EVENT_EXPOSURE:
			OnEventExpo();
			break;
		case MZCAM_EVENT_TEMPTINT:
			OnEventTemptint();
			break;
		case MZCAM_EVENT_IMAGE:
			OnEventImage();
			if (eTriggerLoop == m_eTriggerType)
				Mzcam_Trigger(m_hcam, 1);
			break;
		case MZCAM_EVENT_STILLIMAGE:
			OnEventSnap();
			break;
		}
		return 0;
	...
	void EnumCamera()
	{
		...
		const unsigned cnt = Mzcam_EnumV2(m_arrDev);
		if (0 == cnt)
	...
	{
		Mzcam_GigeEnable(GigeHotplug, m_hWnd);
		EnumCamera();
	...
	{
		if (m_hcam)
			Mzcam_AwbOnce(m_hcam, NULL, NULL);
	}
	...
	void OnAutoExposure(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		...
		{
			int bAutoExposure = 0;
			if (SUCCEEDED(Mzcam_get_AutoExpoEnable(m_hcam, &bAutoExposure)))
			{
				bAutoExposure = !bAutoExposure;
				Mzcam_put_AutoExpoEnable(m_hcam, bAutoExposure);
	...
	void OnVerticalFlip(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		...
		{
			BOOL b = FALSE;
			if (SUCCEEDED(Mzcam_get_VFlip(m_hcam, &b)))
			{
				b = !b;
				Mzcam_put_VFlip(m_hcam, b);
	...
	void OnHorizontalFlip(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		...
		{
			BOOL b = FALSE;
			if (SUCCEEDED(Mzcam_get_HFlip(m_hcam, &b)))
			{
				b = !b;
				Mzcam_put_HFlip(m_hcam, b);
	...
	{
		if (m_hcam && (m_dev.model->flag & MZCAM_FLAG_TEC_ONOFF)) // support set the tec target
		{
	...
	void OnFlash(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		if (m_hcam)
		{
			if (Mzcam_rwc_Flash(m_hcam, MZCAM_FLASH_SIZE, 0, 0, NULL) <= 0)
	...
	void OnRoi(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		...
		{
			CRoiDlg dlg;
			Mzcam_get_Roi(m_hcam, &dlg.xOffset_, &dlg.yOffset_, &dlg.xWidth_, &dlg.yHeight_);
			if (IDOK == dlg.DoModal())
			{
				if (SUCCEEDED(Mzcam_put_Roi(m_hcam, dlg.xOffset_, dlg.yOffset_, dlg.xWidth_, dlg.yHeight_)))
				{
					Mzcam_get_Roi(m_hcam, NULL, NULL, (unsigned*)&m_header.biWidth, (unsigned*)&m_header.biHeight);
	...
	{
		char ver[16] = { 0 };
		if (SUCCEEDED(Mzcam_get_FwVersion(m_hcam, ver)))
		{
			CA2T a2t(ver);
	...
	{
		char ver[16] = { 0 };
		if (SUCCEEDED(Mzcam_get_HwVersion(m_hcam, ver)))
		{
			CA2T a2t(ver);
	...
	{
		char ver[16] = { 0 };
		if (SUCCEEDED(Mzcam_get_FpgaVersion(m_hcam, ver)))
		{
			CA2T a2t(ver);
	...
	void OnTestPattern(UINT /*uNotifyCode*/, int nID, HWND /*wndCtl*/)
	{
		...
		if (val)
			val = val * 2 + 1;
		Mzcam_put_Option(m_hcam, MZCAM_OPTION_TESTPATTERN, val);
	}

	void OnProductionDate(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		char pdate[10] = { 0 };
		if (SUCCEEDED(Mzcam_get_ProductionDate(m_hcam, pdate)))
		{
			CA2T a2t(pdate);
	...
	{
		char sn[32] = { 0 };
		if (SUCCEEDED(Mzcam_get_SerialNumber(m_hcam, sn)))
		{
			CA2T a2t(sn);
	...
	void OnRawformat(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		...
		if (SUCCEEDED(Mzcam_get_RawFormat(m_hcam, &nFourCC, &bitsperpixel)))
		{
			wchar_t str[257];
	...
	void OnTriggerMode(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		...
		{
			int val = 0;
			Mzcam_get_Option(m_hcam, MZCAM_OPTION_TRIGGER, &val);
			if (val == 0)
				val = (m_dev.model->flag & MZCAM_FLAG_TRIGGER_EXTERNAL) ? 2 : 1;
			else
				val = 0;
			Mzcam_put_Option(m_hcam, MZCAM_OPTION_TRIGGER, val);
	...
	void OnTriggerNumber(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		...
		if (IDOK == dlg.DoModal())
		{
			...
			if (m_dev.model->ioctrol > 0)
				Mzcam_IoControl(m_hcam, 0, MZCAM_IOCONTROLTYPE_SET_BURSTCOUNTER, m_nTriggerNumber, NULL);
		}
	}
	...
	void OnTriggerTrigger(UINT /*uNotifyCode*/, int /*nID*/, HWND /*wndCtl*/)
	{
		if (m_hcam)
		{
			...
			{
				m_eTriggerType = eTriggerNumber;
				Mzcam_IoControl(m_hcam, 0, MZCAM_IOCONTROLTYPE_SET_TRIGGERSOURCE, 5, NULL);
				const HRESULT hr = Mzcam_Trigger(m_hcam, m_nTriggerNumber);
				if (E_INVALIDARG == hr)
				{
					if (m_nTriggerNumber > 1)
						AtlMessageBox(m_hWnd, L"MZCAM_FLAG_TRIGGER_SINGLE: only number = 1 supported");
				}
			}
		}
	}
	...
	void OnPreviewResolution(UINT /*uNotifyCode*/, int nID, HWND /*wndCtl*/)
	{
		...
		unsigned eSize = 0;
		if (SUCCEEDED(Mzcam_get_eSize(m_hcam, &eSize)))
		{
			if (eSize != nID - ID_PREVIEW_RESOLUTION0)
			{
				if (SUCCEEDED(Mzcam_Stop(m_hcam)))
				{
					...
					Mzcam_put_eSize(m_hcam, nID - ID_PREVIEW_RESOLUTION0);
					for (unsigned i = 0; i < m_dev.model->preview; ++i)
						UISetCheck(ID_PREVIEW_RESOLUTION0 + i, (nID - ID_PREVIEW_RESOLUTION0 == i) ? 1 : 0);
					UpdateSnapMenu();
					if (SUCCEEDED(Mzcam_get_Size(m_hcam, (int*)&m_header.biWidth, (int*)&m_header.biHeight)))
					{
						UpdateResolutionText();
						UpdateFrameText(L"");
						UpdateExposureTimeText();
						...
						if (SUCCEEDED(Mzcam_StartPullModeWithWndMsg(m_hcam, m_hWnd, MSG_CAMEVENT)))
						{
	...
	void OnSnapResolution(UINT /*uNotifyCode*/, int nID, HWND /*wndCtl*/)
	{
		...
		if (IDOK == dlg.DoModal())
		{
			...
			if (SUCCEEDED(Mzcam_Snap(m_hcam, nID - ID_SNAP_RESOLUTION0)))
			{
				m_nSnapType = 1;
				m_nSnapSeq = 0;
				UpdateSnapMenu();
	...
	void OnSnapnResolution(UINT /*uNotifyCode*/, int nID, HWND /*wndCtl*/)
	{
		...
		{
			if (SUCCEEDED(Mzcam_SnapN(m_hcam, nID - ID_SNAPN_RESOLUTION0, dlg.m_nNum)))
			{
				m_nSnapType = 2;
	...
	void OnOpenDevice(UINT /*uNotifyCode*/, int nID, HWND /*wndCtl*/)
	{
		...
		m_hcam = Mzcam_Open(m_arrDev[idx].id);
		if (m_hcam)
		{
			m_dev = m_arrDev[idx];
			...
			{
				Mzcam_put_Roi(m_hcam, m_xRoiOffset, m_yRoiOffset, m_xRoiWidth, m_yRoiHeight);
				Mzcam_get_Roi(m_hcam, NULL, NULL, (unsigned*)&m_header.biWidth, (unsigned*)&m_header.biHeight);
			}
			else
			{
				Mzcam_get_Size(m_hcam, (int*)&m_header.biWidth, (int*)&m_header.biHeight);
			}

			if (m_bTriggerMode)
			{
				if (m_dev.model->flag & MZCAM_FLAG_TRIGGER_EXTERNAL)
					Mzcam_put_Option(m_hcam, MZCAM_OPTION_TRIGGER, 2);
				else if (m_dev.model->flag & MZCAM_FLAG_TRIGGER_EXTERNAL)
					Mzcam_put_Option(m_hcam, MZCAM_OPTION_TRIGGER, 1);
			}
	...
	void OnEventImage()
	{
		...
		HRESULT hr = Mzcam_PullImageV3(m_hcam, m_pData, 0, m_header.biBitCount, 0, &info);
		if (FAILED(hr))
			return;
	...
	void OnEventSnap()
	{
		...
		MzcamFrameInfoV3 info = { 0 };
		HRESULT hr = Mzcam_PullImageV3(m_hcam, NULL, 1, 24, 0, &info); //first, peek the width and height
		if (SUCCEEDED(hr))
		{
			...
			if (pSnapData)
			{
				hr = Mzcam_PullImageV3(m_hcam, pSnapData, 1, 24, 0, NULL);
				if (SUCCEEDED(hr))
				{
					if (2 == m_nSnapType)
					{
	...
	void OnEventTemptint()
	{
		CStatusBarCtrl statusbar(m_hWndStatusBar);
		wchar_t res[128];
		int nTemp = MZCAM_TEMP_DEF, nTint = MZCAM_TINT_DEF;
		Mzcam_get_TempTint(m_hcam, &nTemp, &nTint);
	...
	void OnEventExpo()
	{
		...
		if (SUCCEEDED(Mzcam_get_ExpoTime(m_hcam, &nTime)) && SUCCEEDED(Mzcam_get_ExpoAGain(m_hcam, &Gain)))
		{
	...
	void OnDeviceChanged()
	{
		...
		else
		{
			...
			int nTemp = MZCAM_TEMP_DEF, nTint = MZCAM_TINT_DEF;
			if (SUCCEEDED(Mzcam_get_TempTint(m_hcam, &nTemp, &nTint)))
			{
				swprintf(res, L"Temp = %d, Tint = %d", nTemp, nTint);
				statusbar.SetText(2, res);
			}
		...
		UIEnable(ID_TECTARGET, (m_hcam && (m_dev.model->flag & MZCAM_FLAG_TEC_ONOFF)) ? TRUE : FALSE);
	...
	void UpdateResolutionText()
	{
		...
		if (SUCCEEDED(Mzcam_get_Roi(m_hcam, &xOffset, &yOffset, &nWidth, &nHeight)))
		{
	...
	void UpdateFrameText(const MzcamFrameInfoV3& info)
	{
		wchar_t str[256];
		if (m_dwLastTick != m_dwStartTick)
		{
			if (info.flag & (MZCAM_FRAMEINFO_FLAG_SEQ | MZCAM_FRAMEINFO_FLAG_TIMESTAMP))
