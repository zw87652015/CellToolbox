...
BEGIN_MESSAGE_MAP(CdemomonoDlg, CDialog)
	...
	ON_WM_HSCROLL()
	ON_COMMAND_RANGE(IDM_SNAP_RESOLUTION, IDM_SNAP_RESOLUTION + MZCAM_MAX, &CdemomonoDlg::OnSnapResolution)
END_MESSAGE_MAP()
...
void CdemomonoDlg::OnBnClickedButton1()
{
	...
	int n = (int)Mzcam_get_ResolutionNumber(m_hcam);
	if (n > 0)
	{
		TCHAR txt[128];
		int nWidth, nHeight;
	...
	m_maxBitDepth = Mzcam_get_MaxBitDepth(m_hcam);

	if ((8 == m_maxBitDepth) || IsDlgButtonChecked(IDC_CHECK2))
	{
		m_bBitDepth = false;
		CheckDlgButton(IDC_CHECK2, 0);
		Mzcam_put_Option(m_hcam, MZCAM_OPTION_RGB, 3);
		Mzcam_put_Option(m_hcam, MZCAM_OPTION_BITDEPTH, 0);
	}
	else
	{
		m_bBitDepth = true;
		Mzcam_put_Option(m_hcam, MZCAM_OPTION_RGB, 4);
		Mzcam_put_Option(m_hcam, MZCAM_OPTION_BITDEPTH, 1);
	}
...
void CdemomonoDlg::OnEventImage()
{
	...
	HRESULT hr = Mzcam_PullImageV3(m_hcam, m_pImageData, 0, m_bBitDepth ? 16 : 8, 0, &info);
	if (SUCCEEDED(hr))
	{
...
{
	MzcamFrameInfoV3 info = { 0 };
	HRESULT hr = Mzcam_PullImageV3(m_hcam, NULL, 1, m_bBitDepth ? 16 : 8, 0, &info);
	if (SUCCEEDED(hr))
	{
		void* pData = malloc(TDIBWIDTHBYTES(info.width * 16) * info.height);
		hr = Mzcam_PullImageV3(m_hcam, pData, 1, m_bBitDepth ? 16 : 8, 0, NULL);
		if (SUCCEEDED(hr))
		{
