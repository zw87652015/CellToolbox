...
public:
	...
	: m_expoTimeRange(expoTimeRange), m_expoGainRange(expoGainRange), m_expoTime(0), m_expoGain(MZCAM_EXPOGAIN_MIN), m_delayTime(0)
	{
	}
...
public:
	enum { IDD = IDD_CONFIG };
	CConfigDlg(HMzcam hcam, const MzcamModelV2* pModel)
		: m_hcam(hcam), m_pModel(pModel), m_area(5), m_bin(1), m_temp(0), m_scale(0)
	{
		...
		Mzcam_get_ExpTimeRange(m_hcam, &m_expoTimeRange[0], &m_expoTimeRange[1], nullptr);
		Mzcam_get_ExpoAGainRange(m_hcam, &m_expoGainRange[0], &m_expoGainRange[1], nullptr);
	}
...
private:
	LRESULT OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		if (E_NOTIMPL == Mzcam_get_Option(m_hcam, MZCAM_OPTION_BITDEPTH, nullptr)) // support bitdepth
		{
		...
		if (E_NOTIMPL == Mzcam_get_Temperature(m_hcam, nullptr)) // support get the temperature of the sensor
...

		{
			...
			if (m_pModel->flag & MZCAM_FLAG_FAN)
			{
				box.AddString(L"OFF");
...

		{
			...
			if (m_pModel->flag & (MZCAM_FLAG_CG | MZCAM_FLAG_CGHDR))
			{
				DWORD val = 0, maxval = 1;
				box.AddString(L"LCG");
				box.AddString(L"HCG");
				if (m_pModel->flag & MZCAM_FLAG_CGHDR)
				{
					box.AddString(L"HDR");
					maxval = 2;
				}
		...
		if (m_pModel->flag & MZCAM_FLAG_TEC_ONOFF)
		{
			DWORD val = 0;
			m_regkey.QueryDWORDValue(L"tec", val);
			CheckDlgButton(IDC_CHECK3, val ? 1 : 0);
			val = MZCAM_TEC_TARGET_DEF;
			m_regkey.QueryDWORDValue(L"tectarget", val);
			int tectarget = (int)val;
			if (tectarget < MZCAM_TEC_TARGET_MIN)
				tectarget = MZCAM_TEC_TARGET_MIN;
			else if (tectarget > MZCAM_TEC_TARGET_MAX)
				tectarget = MZCAM_TEC_TARGET_MAX;
		...
		if (m_pModel->flag & MZCAM_FLAG_HEAT)
		{
			CUpDownCtrl ctrl(GetDlgItem(IDC_SPIN8));
			DWORD heat = 0, maxheat = 0;
			Mzcam_get_Option(m_hcam, MZCAM_OPTION_HEAT_MAX, (int*)&maxheat);
	...
	LRESULT OnOK(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (m_pModel->flag & MZCAM_FLAG_FAN)
		{
			...
			Mzcam_put_Option(m_hcam, MZCAM_OPTION_FAN, c);
		...
		if (m_pModel->flag & MZCAM_FLAG_TEC_ONOFF)
		{
			...
			Mzcam_put_Option(m_hcam, MZCAM_OPTION_TEC, bEnable);
			...
			if (bEnable)
			{
				...
				if ((n < MZCAM_TEC_TARGET_MIN) || (n > MZCAM_TEC_TARGET_MAX))
				{
					AtlMessageBox(m_hWnd, (LPCTSTR)FormatString(L"Tec target out of range [%.1f, %.1f].", MZCAM_TEC_TARGET_MIN / 10.0, MZCAM_TEC_TARGET_MAX / 10.0), (LPCTSTR)nullptr, MB_OK | MB_ICONWARNING);
					return 0;
				}
				Mzcam_put_Option(m_hcam, MZCAM_OPTION_TECTARGET, n);
		...
		if (m_pModel->flag & MZCAM_FLAG_HEAT)
		{
			...
			Mzcam_put_Option(m_hcam, MZCAM_OPTION_HEAT, heat);
		}
		...
		if (m_pModel->flag & (MZCAM_FLAG_CG | MZCAM_FLAG_CGHDR))
		{
			CComboBox box(GetDlgItem(IDC_COMBO5));
			const int c = box.GetCurSel();
			Mzcam_put_Option(m_hcam, MZCAM_OPTION_CG, c);
		...
		if (E_NOTIMPL != Mzcam_get_Temperature(m_hcam, nullptr))
		{
		...
		if (m_bin > 1)
		{
			...
			Mzcam_put_Option(m_hcam, MZCAM_OPTION_BINNING, m_bin);
		}
		{
			const unsigned r = m_area / 2;
			int width = 0, height = 0;
			Mzcam_get_FinalSize(m_hcam, &width, &height);
			if (IsDlgButtonChecked(IDC_RADIO1))
			{
		...
		if (E_NOTIMPL != Mzcam_get_Option(m_hcam, MZCAM_OPTION_BITDEPTH, nullptr)) // support bitdepth
		{
			...
			Mzcam_put_Option(m_hcam, MZCAM_OPTION_BITDEPTH, bCheck);
			if (bCheck)
			{
...
public:
	...
	LRESULT OnStart(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		if (nullptr == m_hcam)
		{
			...
			else
			{
				...
				const unsigned num = Mzcam_EnumV2(arr);
				if (num <= 0)
	...
	void OnTimer(UINT_PTR nIDEvent)
	{
		...
		case TIMER_TEMP:
			...
			{
				short temp = 0;
				if (SUCCEEDED(Mzcam_get_Temperature(m_hcam, &temp)))
				{
					int val = temp;
...
private:
	void OpenCamera(const wchar_t* camId)
	{
		...
		m_pModel = Mzcam_query_Model(m_hcam);
		Mzcam_put_eSize(m_hcam, 0); // always use the maximum resolution
		Mzcam_put_Option(m_hcam, MZCAM_OPTION_RAW, 1);
		Mzcam_put_HZ(m_hcam, 2);
		Mzcam_put_AutoExpoEnable(m_hcam, 0); // always disable auto exposure
		...
		else
		{
			...
			unsigned nFourCC;
			Mzcam_get_RawFormat(m_hcam, &nFourCC, &m_bitdepth);
			m_ymax = 1 << m_bitdepth;
			if (dlg.m_bin & 0x40)
			...
			int w = 0, h = 0;
			Mzcam_get_FinalSize(m_hcam, &w, &h);
			...
			m_curGraph = &m_vecGraph[0];
			m_curWnd = m_curGraph;
			if ((E_NOTIMPL != Mzcam_get_Temperature(m_hcam, nullptr)) && dlg.m_temp)
			{
				m_tabCtrl.AddItem(L"Temperature");
			...
			m_idxExpo = 0;
			Mzcam_put_ExpoTime(m_hcam, m_vecExpo[m_idxExpo].expoTime);
			Mzcam_put_ExpoAGain(m_hcam, m_vecExpo[m_idxExpo].expoGain);
			Mzcam_StartPullModeWithWndMsg(m_hcam, m_hWnd, MSG_CAMERA);
			if (m_bTriggerMode)
			{
				Mzcam_Trigger(m_hcam, 1);
				m_bWantTigger = false;
	...
	template<typename T>
	void GetData(int* arr, const T* pData, const MzcamFrameInfoV3& info)
	{
	...
	void OnEventImage()
	{
		...
		const HRESULT hr = Mzcam_PullImageV3(m_hcam, m_pRawData, 0, 0, 0, &info);
		if (SUCCEEDED(hr))
		{
			if (m_bTriggerMode)
				bAdd = true;
...
static int Run()
{
	...
	if (frmMain.CreateEx() == nullptr)
		return 0;
	Mzcam_GigeEnable(nullptr, nullptr);
