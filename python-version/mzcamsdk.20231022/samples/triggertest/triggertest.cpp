...
private:
	...
	LRESULT OnButton2(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		else
		{
			...
			Mzcam_get_ExpTimeRange(m_hcam, &minTime, &maxTime, &defTime);
			...
			Mzcam_put_ExpoTime(m_hcam, expoTime);
			if (FAILED(Mzcam_StartPullModeWithCallback(m_hcam, StaticCameraCallback, this)))
			{
...
private:
	...
	void CameraCallback(unsigned nEvent)
	{
		if (MZCAM_EVENT_IMAGE == nEvent)
		{
			...
			const HRESULT hr = Mzcam_PullImageV3(m_hcam, pdata, 0, 24, 0, nullptr);
			if (SUCCEEDED(hr))
			{
				++m_image;
				if (m_save)
	...
	void OpenCamera(const wchar_t* camId, const MzcamModelV2* model)
	{
		...
		m_model = model;
		if (0 == (m_model->flag & MZCAM_FLAG_TRIGGER_SOFTWARE))
		{
