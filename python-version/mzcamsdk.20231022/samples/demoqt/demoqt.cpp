...
MainWidget::MainWidget(QWidget* parent)
    ...
    , m_res(0), m_temp(MZCAM_TEMP_DEF), m_tint(MZCAM_TINT_DEF), m_count(0)
{
    ...
    QGroupBox* gboxres = new QGroupBox("Resolution");
    {
        ...
        connect(m_cmb_res, QOverload<int>::of(&QComboBox::currentIndexChanged), this, [this](int index)
        {
            ...
            {
                Mzcam_put_eSize(m_hcam, static_cast<unsigned>(m_res));
                startCamera();
            }
        });
    ...
    QGroupBox* gboxexp = new QGroupBox("Exposure");
    {
        ...
        connect(m_cbox_auto, &QCheckBox::stateChanged, this, [this](bool state)
        {
            if (m_hcam)
            {
                Mzcam_put_AutoExpoEnable(m_hcam, state ? 1 : 0);
        ...
        connect(m_slider_expoTime, &QSlider::valueChanged, this, [this](int value)
        {
            if (m_hcam)
            {
                ...
                if (!m_cbox_auto->isChecked())
                   Mzcam_put_ExpoTime(m_hcam, value);
            }
        });
        connect(m_slider_expoGain, &QSlider::valueChanged, this, [this](int value)
        {
            if (m_hcam)
            {
                ...
                if (!m_cbox_auto->isChecked())
                    Mzcam_put_ExpoAGain(m_hcam, value);
            }
        });
    ...
    QGroupBox* gboxwb = new QGroupBox("White balance");
    {
        ...
        {
            Mzcam_AwbOnce(m_hcam, nullptr, nullptr);
        });
        m_lbl_temp = new QLabel(QString::number(MZCAM_TEMP_DEF));
        m_lbl_tint = new QLabel(QString::number(MZCAM_TINT_DEF));
        m_slider_temp = new QSlider(Qt::Horizontal);
        m_slider_tint = new QSlider(Qt::Horizontal);
        m_slider_temp->setRange(MZCAM_TEMP_MIN, MZCAM_TEMP_MAX);
        m_slider_temp->setValue(MZCAM_TEMP_DEF);
        m_slider_tint->setRange(MZCAM_TINT_MIN, MZCAM_TINT_MAX);
        m_slider_tint->setValue(MZCAM_TINT_DEF);
        ...
        connect(m_slider_temp, &QSlider::valueChanged, this, [this](int value)
        {
            m_temp = value;
            if (m_hcam)
                Mzcam_put_TempTint(m_hcam, m_temp, m_tint);
    ...
    connect(this, &MainWidget::evtCallback, this, [this](unsigned nEvent)
    {
        ...
        if (m_hcam)
        {
            if (MZCAM_EVENT_IMAGE == nEvent)
                handleImageEvent();
            else if (MZCAM_EVENT_EXPOSURE == nEvent)
                handleExpoEvent();
            else if (MZCAM_EVENT_TEMPTINT == nEvent)
                handleTempTintEvent();
            else if (MZCAM_EVENT_STILLIMAGE == nEvent)
                handleStillImageEvent();
            else if (MZCAM_EVENT_ERROR == nEvent)
            {
                closeCamera();
                QMessageBox::warning(this, "Warning", "Generic error.");
            }
            else if (MZCAM_EVENT_DISCONNECTED == nEvent)
            {
                closeCamera();
    ...
    connect(m_timer, &QTimer::timeout, this, [this]()
    {
        ...
        if (m_hcam && SUCCEEDED(Mzcam_get_FrameRate(m_hcam, &nFrame, &nTime, &nTotalFrame)) && (nTime > 0))
...
void MainWidget::startCamera()
{
    ...
    Mzcam_get_ExpTimeRange(m_hcam, &uimin, &uimax, &uidef);
    m_slider_expoTime->setRange(uimin, uimax);
    Mzcam_get_ExpoAGainRange(m_hcam, &usmin, &usmax, &usdef);
    m_slider_expoGain->setRange(usmin, usmax);
    if (0 == (m_cur.model->flag & MZCAM_FLAG_MONO))
        handleTempTintEvent();
    handleExpoEvent();
    if (SUCCEEDED(Mzcam_StartPullModeWithCallback(m_hcam, eventCallBack, this)))
    {
        m_cmb_res->setEnabled(true);
        m_cbox_auto->setEnabled(true);
        m_btn_autoWB->setEnabled(0 == (m_cur.model->flag & MZCAM_FLAG_MONO));
        m_slider_temp->setEnabled(0 == (m_cur.model->flag & MZCAM_FLAG_MONO));
        m_slider_tint->setEnabled(0 == (m_cur.model->flag & MZCAM_FLAG_MONO));
        m_btn_open->setText("Close");
        m_btn_snap->setEnabled(true);

        int bAuto = 0;
        Mzcam_get_AutoExpoEnable(m_hcam, &bAuto);
...
{
    m_hcam = Mzcam_Open(m_cur.id);
    if (m_hcam)
    {
        Mzcam_get_eSize(m_hcam, (unsigned*)&m_res);
        ...
        Mzcam_put_Option(m_hcam, MZCAM_OPTION_BYTEORDER, 0); //Qimage use RGB byte order
        Mzcam_put_AutoExpoEnable(m_hcam, 1);
        startCamera();
    }
}
...
void MainWidget::onBtnOpen()
{
    ...
    {
        MzcamDeviceV2 arr[MZCAM_MAX] = { 0 };
        unsigned count = Mzcam_EnumV2(arr);
        if (0 == count)
...
void MainWidget::handleImageEvent()
{
    ...
    if (SUCCEEDED(Mzcam_PullImage(m_hcam, m_pData, 24, &width, &height)))
    {
...
void MainWidget::handleExpoEvent()
{
    ...
    Mzcam_get_ExpoTime(m_hcam, &time);
    Mzcam_get_ExpoAGain(m_hcam, &gain);
    {
...
{
    unsigned width = 0, height = 0;
    if (SUCCEEDED(Mzcam_PullStillImage(m_hcam, nullptr, 24, &width, &height))) // peek
    {
        std::vector<uchar> vec(TDIBWIDTHBYTES(width * 24) * height);
        if (SUCCEEDED(Mzcam_PullStillImage(m_hcam, &vec[0], 24, &width, &height)))
        {
