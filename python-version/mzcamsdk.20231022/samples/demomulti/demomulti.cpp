...
MzcamDeviceV2 g_dev[MZCAM_MAX] = { 0 };
struct ctxCam {
    MzcamDeviceV2* dev;
    HMzcam hcam;
    void* data;
...
static void __stdcall EventCallback(unsigned nEvent, void* pCallbackCtx)
{
    ...
    if (MZCAM_EVENT_IMAGE == nEvent)
    {
        ...
        const HRESULT hr = Mzcam_PullImageV3(pctx->hcam, pctx->data, 0, 24, 0, &info);
        if (FAILED(hr))
...
int main(int, char**)
{
    unsigned num = Mzcam_EnumV2(g_dev);
    if (0 == num)
    {
    ...
    {
        g_ctx[i].dev = &g_dev[i];
        g_ctx[i].hcam = Mzcam_Open(g_dev[i].id);
        if (NULL == g_ctx[i].hcam)
            _tprintf(_T("%s: open failed\n"), g_ctx[i].dev->displayname);
        else
        {
            int nWidth = 0, nHeight = 0;
            HRESULT hr = Mzcam_get_Size(g_ctx[i].hcam, &nWidth, &nHeight);
            if (FAILED(hr))
            ...
            else
            {
                ...
                {
                    hr = Mzcam_StartPullModeWithCallback(g_ctx[i].hcam, EventCallback, (void*)&g_ctx[i]);
                    if (FAILED(hr))
