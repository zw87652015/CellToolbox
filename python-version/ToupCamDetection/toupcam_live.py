"""
ToupCam Live Stream - Main Application Class
This module contains the main ToupCameraLive class that orchestrates the entire application.
"""
...
class ToupCameraLive:
    """Main application class for ToupCam live stream with cell detection"""
    
    def __init__(self, root):
        self.root = root
        self.root.title("ToupCam Live Stream")
    ...
    def create_camera_panel(self, parent):
        ...
        exposure_button = ttk.Button(
            camera_frame,
            text="Exposure Control",
            command=self.open_exposure_control
        )
        exposure_button.pack(fill=tk.X, padx=5, pady=5)
    ...
    def render_loop(self):
        """Optimized rendering loop with frame skipping and minimal latency"""
        frame_skip_counter = 0
        target_display_fps = 60  # Higher display FPS for smoother experience
        frame_interval = 1.0 / target_display_fps
        ...
        while self.render_running:
            try:
                ...
                # Only process frames at target display rate to reduce CPU load
                if current_time - last_render_time >= frame_interval:
                    # Get latest frame (skip intermediate frames for lower latency)
                    success, frame = self.camera_manager.get_frame()
                    if success and frame is not None:
                        # Schedule UI update on main thread with minimal delay
                        self.root.after_idle(lambda f=frame: self.update_frame(f))
                        last_render_time = current_time
                        frame_skip_counter = 0
                    else:
                        frame_skip_counter += 1
                        # If no new frames for too long, reduce check frequency
                ...
                if current_time - self.last_fps_time >= 1.0:
                    camera_fps = self.camera_manager.frame_count / (current_time - self.last_fps_time)
                    self.fps = camera_fps
                    # Use after_idle for non-critical UI updates
                    self.root.after_idle(lambda: self.fps_var.set(f"FPS: {camera_fps:.1f}"))
                    self.camera_manager.frame_count = 0
                    self.last_fps_time = current_time
                
                # Minimal sleep to prevent excessive CPU usage
                time.sleep(0.001)  # 1ms sleep, much shorter than original 33ms
                
            except Exception as e:
                print(f"Error in render loop: {e}")
                time.sleep(0.01)  # Shorter error recovery sleep
    ...
    def cell_detection_loop(self):
        """Optimized cell detection loop with lower priority and frame sharing"""
        # Set lower thread priority to avoid interfering with display
        import threading
        current_thread = threading.current_thread()
        
        # Cache frame to avoid competing with display thread
        cached_frame = None
        frame_cache_time = 0
        cache_duration = 0.1  # Cache frame for 100ms to reduce camera access conflicts
        
        while self.cell_detection_active:
            try:
                current_time = time.time()
                if current_time - self.last_detection_time >= self.cell_detection_interval:
                    # Use cached frame if recent, otherwise get new frame
                    if (cached_frame is None or 
                        current_time - frame_cache_time > cache_duration):
                        success, frame = self.camera_manager.get_frame()
                        if success and frame is not None:
                            cached_frame = frame.copy()  # Make a copy for processing
                    ...
                    if cached_frame is not None:
                        ...
                        if self.show_debug:
                            self.detected_cells, debug_image = self.cell_detector.detect_cells(cached_frame, aoi_coords, return_debug=True)
                            # Update debug window with binary segmentation result
                            if debug_image is not None:
                                self.root.after_idle(lambda: self.update_debug_window(debug_image))
                        else:
                            self.detected_cells = self.cell_detector.detect_cells(cached_frame, aoi_coords)
                        
                        # Update cell count using after_idle for lower priority
                        cell_count = len(self.detected_cells)
                        self.root.after_idle(lambda count=cell_count: self.cell_count_var.set(f"Cells: {count}"))
                        
                        # Reduced debug output frequency to avoid console spam
                        if current_time - getattr(self, '_last_debug_time', 0) > 2.0:  # Every 2 seconds
                            if cell_count > 0:
                ...
                # Longer sleep to reduce CPU usage and avoid interfering with display
                time.sleep(0.02)  # 20ms sleep instead of 10ms
