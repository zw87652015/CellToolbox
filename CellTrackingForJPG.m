...
for i = 1:n_runs
            ...
            if 50<L(i) && L(i)<300
                if 0.8<C(i) && C(i)<1.8
                    ...
                    if (h>w && 1.5*w>h) || (w>h && 1.5*h>w) || (h==w)
                        rectangle('Position', [cmin rmin w h], 'EdgeColor', 'r');
                    end
                end
            end
        end
    end
    % Save the figure with rectangles
    % imageFileName = fullfile(processingFolder, sprintf('19-FinalResult.png'));
    ...
    times(i) = toc;  % End timing and store
...
avg_time = mean(times);
std_time = std(times);
fprintf('\nAverage processing time over %d runs: %.2f ± %.2f seconds\n', n_runs, avg_time, std_time);